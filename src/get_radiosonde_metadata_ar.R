...
# Loop through mlist
for (name in names(mlist.ar))
{
  print(name)
  # Get dataframe
  df = mlist.ar[[name]]
  
  n_reps = nrow(val.ar[val.ar$radiosonde == tolower(name),])
  print(n_reps)
  ...
  if (all(df$RADI_SNDE_TYPE == df$RADI_SNDE_TYPE[1]))
  {
    ...
    {
      val.ar[val.ar$radiosonde == tolower(name),]$rtype = rep(df$RADI_SNDE_TYPE[1], n_reps)
    }
  } else
  {
  ...
  if (all(df$TRCKG_SYTM == df$TRCKG_SYTM[1]))
  {
    ...
    {
      val.ar[val.ar$radiosonde == tolower(name),]$tracksys = rep(df$TRCKG_SYTM[1], n_reps)
    }
  } else
  {
    print("Oooh different!")
  ...
  if (all(df$RADTN_CORTN == df$RADTN_CORTN[1]))
  {
    ...
    {
      val.ar[val.ar$radiosonde == tolower(name),]$radcorr = rep(df$RADTN_CORTN[1], n_reps)
    }
  } else
  {
    print("Oooh different!")
